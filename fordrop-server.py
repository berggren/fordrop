...
parser.add_option('-u', '--username', action='store', dest='username', default=config.get("fordrop-rest-server", "username"), help='username for fordropcore-server')
parser.add_option('-k', '--api-key', action='store', dest='api_key', default=config.get("fordrop-rest-server", "api-key"), help='api-key for server')
parser.add_option('-j', '--jid', action='store', dest='jid', default=config.get("fordrop-rest-server", "jid"), help='jid, XMPP username')
parser.add_option('-p', '--password', action='store', dest='password', default=config.get("fordrop-rest-server", "password"), help='password, XMPP password')
parser.add_option('-s', '--server', action='store', dest='server', default=config.get("fordrop-rest-server", "server"), help='XMPP server')
parser.add_option('--pubsub', action='store', dest='pubsub', default=config.get("fordrop-rest-server", "pubsub_service"), help='XMPP pubsub service')
parser.add_option('--bind-address', action='store', dest='bind_address', default=config.get("fordrop-rest-server", "bind_address"), help='Address to bind to (HTTP server)')
parser.add_option('--bind-port', action='store', dest='bind_port', default=config.getint("fordrop-rest-server", "bind_port"), help='Port to bind to (HTTP server)')
...
class NodeResource:
    ...
    def POST(self):
        ...
        self.pubsub.create_node(options.pubsub, body.get('name'), config=config)
...
class SubscriberResource:
    ...
    def POST(self):
        body = json.loads(cherrypy.request.body.read(int(cherrypy.request.headers['Content-Length'])))
        self.pubsub.subscribe(jid=body.get('service'), node=body.get('node'), subscribee=options.jid)
        cherrypy.response.status = 201

    @cherrypy.tools.login_required()
    def DELETE(self):
        body = json.loads(cherrypy.request.body.read(int(cherrypy.request.headers['Content-Length'])))
        self.pubsub.unsubscribe(jid=body.get('service'), node=body.get('node'), subscribee=options.jid)
        cherrypy.response.status = 200
